...
class WebTrailApp:
    ...
    def _build_control_panel(self, parent: ttk.Frame):
        ...
        # 输出目录选择
        out_card = ttk.Frame(parent, style="Card.TFrame", padding=12)
        out_card.pack(fill="x", pady=(0, 8))

        ttk.Label(out_card, text="报告保存位置", style="H2.TLabel").pack(anchor="w")
        ...
        ttk.Button(out_row, text="浏览...", command=self._choose_output_dir,
    ...
    def _start_analysis(self):
        ...
        if not out_dir:
            messagebox.showwarning("提示", "请指定报告保存位置。")
            return
        try:
            Path(out_dir).mkdir(parents=True, exist_ok=True)
        except OSError:
            messagebox.showwarning("提示", f"无法创建目录: {out_dir}")
            return
        ...
        thread = threading.Thread(
            target=self._run_analysis, args=(selected, out_dir), daemon=True
        )
        thread.start()

    def _run_analysis(self, selected: list[str], out_dir: str):
        try:
            extractors = create_extractors(set(selected))
            if not extractors:
                self.root.after(0, lambda: self._analysis_done(False, "未检测到浏览器数据。", out_dir))
                return
            ...
            if not self.records:
                self.root.after(0, lambda: self._analysis_done(False, "未提取到任何痕迹记录。", out_dir))
                return
            ...
            self.root.after(0, lambda: self._analysis_done(False, f"分析失败: {e}", out_dir))

    def _analysis_done(self, success: bool, msg: str, out_dir: str = ""):
        self.running = False
        self.progress.stop()
        ...
        if success:
            ...
            try:
                report_dir = write_report(
                    self.records, self.profile_result, {}, Path(out_dir)
                )
                self.status_var.set(f"分析完成 · 共 {len(self.records)} 条记录 · 已保存: {report_dir}")
            except Exception as e:
                self.status_var.set(f"分析完成 · 共 {len(self.records)} 条记录 · 保存失败: {e}")
        else:
