...
- **顶部** — 标题栏，自动检测 Chrome / Edge / Firefox 安装状态（可多选）
- **左上** — "开始分析"按钮，点击后异步执行，UI 不冻结
- **左下** — 报告保存位置输入框 + "浏览..."目录选择器，支持自定义输出路径
- **右侧** — 5 个结果选项卡（概览、TOP 域名、活跃时段、访问类别、风险指标）
- **底部** — 状态栏，分析完成后自动保存报告并显示保存路径

无需在分析后手动导出——选择好保存位置点击"开始分析"即可，报告自动写入指定目录。
